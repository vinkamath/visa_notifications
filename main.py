...
class App:
    ...
    def init_app(self) -> None:
        ...
        load_dotenv()
        self.secrets = AppSecrets(self.config)

        # Configure logging
        logging.basicConfig(level=logging.INFO)

    async def fetch_messages(self, client: TelegramClient, bot_client: TelegramClient, source_group_entity: Any) -> None:
        ...
        if self.config.test_mode:
            ...
            target_channel = self.secrets.test_channel_chat_id
        else:
            target_channel = self.secrets.broadcast_channel_chat_id
        ...
        try:
            while True:
                ...
                async for message in client.iter_messages(source_group_entity, min_id=last_message_id, limit=self.config.message_fetch_limit):
                ...
                while messages:
                    ...
                    try:
                        ...
                        await bot_client.send_message(entity=target_channel, message=message.message, silent=False)
                        self.logger.info(f"✅ #{message.id} {message.message}")
    ...
    async def main(self):
        ...
        try:
            ...
            source_group_entity = await client.get_entity(self.config.source_group_name)
            self.logger.debug(f'Connected to source group: {source_group_entity}')
        except errors.FloodWaitError as e:
        ...
        await self.fetch_messages(client, bot_client, source_group_entity)
